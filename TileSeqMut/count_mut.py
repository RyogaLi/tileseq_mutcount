...
class readSam(object):
    ...
    def multi_core(self):
        ...
        with open(self._r1, "r") as f1:

            read_Byte = f1.tell()
            for line in f1:
                jobs.append(pool.apply_async(self.process_wrapper, (read_Byte)))
                read_Byte = f1.tell()
                # r2_Byte = f1.tell()
        # wait for all jobs to finish
        for job in jobs:

            off, nomut, unmap, hgvs, outside_mut = job.get()
            off_read += off
            read_nomut += nomut
            un_map += unmap
            if len(hgvs) != 0:
    ...
    def process_wrapper(self, line_byte):
        """
        ...
        with open(self._r1, "r") as f1, open(self._r2, "r") as f2:
            f1.seek(line_byte)
            f2.seek(line_byte)

            line_r1 = f1.readline().split()
            line_r2 = f2.readline().split()
        ...
        return off_read, read_nomut, un_map, hgvs, outside_mut
